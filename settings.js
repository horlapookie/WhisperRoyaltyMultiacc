export const settings = {
    // Bot Configuration
    botName: 'yourhïghness',
    version: '1.5',
    prefix: '.',

    // Multi-Session Configuration
    sessions: [
        {
            id: 'session1',
            ownerNumber: '2349133507428',
            ownerNumbers: ['2349133507428@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session2',
            ownerNumber: '2349029099539',
            ownerNumbers: ['2349029099539@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session3',
            ownerNumber: '2349122222622',
            ownerNumbers: ['2349122222622@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session4',
            ownerNumber: '2348142310854',
            ownerNumbers: ['2348142310854@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session5',
            ownerNumber: '2348143799609',
            ownerNumbers: ['2348143799609@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session6',
            ownerNumber: '2349029099539',
            ownerNumbers: ['2349029099539@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session7',
            ownerNumber: '2349168588245',
            ownerNumbers: ['2349168588245@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session8',
            ownerNumber: '2348028720274',
            ownerNumbers: ['2348028720274@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session9',
            ownerNumber: '27673268213',
            ownerNumbers: ['27673268213@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session10',
            ownerNumber: '27673268213',
            ownerNumbers: ['27673268213@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session11',
            ownerNumber: '2348028720274',
            ownerNumbers: ['2348028720274@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session12',
            ownerNumber: 'YOUR_TWELFTH_NUMBER',
            ownerNumbers: ['YOUR_TWELFTH_NUMBER@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session13',
            ownerNumber: 'YOUR_THIRTEENTH_NUMBER',
            ownerNumbers: ['YOUR_THIRTEENTH_NUMBER@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session14',
            ownerNumber: 'YOUR_FOURTEENTH_NUMBER',
            ownerNumbers: ['YOUR_FOURTEENTH_NUMBER@s.whatsapp.net'],
            prefix: '.'
        },
        {
            id: 'session15',
            ownerNumber: 'YOUR_FIFTEENTH_NUMBER',
            ownerNumbers: ['YOUR_FIFTEENTH_NUMBER@s.whatsapp.net'],
            prefix: '.'
        }
    ],

    // Legacy support (will use first enabled session)
    ownerNumber: '2349133507428',
    ownerNumbers: ['2349133507428@s.whatsapp.net'],
    sessionBase64: "",

    // API Keys
    geminiApiKey: process.env.GEMINI_API_KEY || 'AIzaSyArdMt3se0P2U5PCWjprpBZlzGZ2bHJklg',
    youtubeApiKey: process.env.YOUTUBE_API_KEY || 'AIzaSyBvVo--Jslb084-F8ATSWgsgqOl2JVh660',
    geniusApiKey: "NrGLCWeRCNlny8qtUzXhxalvAwWWjcjWdwyCe3aUrXJZLlzs3lwSd5ddu_Iy3q5O",
    openaiApiKey: "",
    deepaiApiKey: process.env.DEEPAI_API_KEY || '4adb02ce-adbb-44b1-be08-f887c35ad486',
    falsecallerId: 'a1i0x--L2j_d8lF4BTFZ7e3p0t',
    falsecallerId: process.env.TRUECALLER_ID || '',

    // Profile Pictures
    profilePics: [
        'https://files.catbox.moe/mq8b1n.png',
        'https://files.catbox.moe/dm7w9d.jpeg',
        'https://files.catbox.moe/0j5tnz.jpeg',
        'https://files.catbox.moe/b7wnah.jpeg',
        'https://files.catbox.moe/oo7yfn.jpeg',
        'https://files.catbox.moe/57l61y.jpeg',
        'https://files.catbox.moe/q64syc.jpeg'
    ],

    // Anti-spam settings
    antiSpam: {
        enabled: false,
        maxMessages: 5,
        timeWindow: 60000,
        cooldownTime: 30000
    }
};