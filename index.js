...
// Initialize multi-session bot manager
...
Type \`${settings.prefix}help\` to explore commands 🚀`,
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        // Improved owner recognition - check multiple owner numbers and formats
        const ownerNumbers = settings.ownerNumbers || [settings.ownerNumber];
        ...
        if (!botState.isPublic && !isOwner) {
            console.log(`Private mode: Ignoring message from ${displayId} (not owner)`);
            return;
        }
        ...
        if (messageText.startsWith(settings.prefix)) {
        ...
        } else if ((botState.chatbotEnabled || global.chatbot2Enabled) && !isGroup && !isOwner && messageText.trim() !== '' && (botState.isPublic || isOwner)) {
            ...
            // Try chatbot2 first if enabled
            if (global.chatbot2Enabled) {
                try {
                    const { handleChatbot2Response } = await import('./horlapookie/chatbot2.js');
                    const handled = await handleChatbot2Response(sock, msg, messageText, {
                        from: from,
                        sender: sender,
                        settings: settings,
                        isOwner: isOwner,
                    ...
                    if (handled) return; // Exit if chatbot2 handled the message
                } catch (error) {
                    console.error('Chatbot2 error:', error);
                }
            }

            // Fallback to original chatbot if enabled and chatbot2 didn't handle it
...
async function init() {
    await loadBotState();
    //await startBot(); // This line commented to use multi-session
    try {
        console.log('🔄 Initializing multi-session bot manager...');

        // Start all enabled sessions
        await botManager.initializeAllSessions();

        console.log('✅ Bot initialization complete!');
        console.log('🤖 All sessions are now running...');
        ...
        setTimeout(() => {
            init().catch(() => {
                console.error('❌ Critical failure - exiting');
                process.exit(1);
            });
        }, 10000);
    }
...
// Export restart function globally, to allow it being called from other modules
        ...
        const server = http.createServer((req, res) => {
            if (req.url === '/health' || req.url === '/') {
                ...
                res.end(JSON.stringify({
                    ...
                    sessions: global.botManager ? global.botManager.getSessionStatus() : 'initializing',
